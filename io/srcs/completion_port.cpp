...
completion_port::completion_port(basic_engine<this_type> &e)
    ...
    if (_handle == INVALID_HANDLE_VALUE) {
        throw std::system_error(cx::system_error(),
...
bool completion_port::bind(const descriptor_type &descriptor,
    ...
    if (CreateIoCompletionPort(descriptor->fd.h, _handle,
...
int completion_port::run(const std::chrono::milliseconds &wait_ms) {
    ...
    ULONG_PTR key = 0;
    BOOL ret = GetQueuedCompletionStatus(_handle, &bytes_transferred, &key, &ov,
                                         static_cast<DWORD>(wait_ms.count()));

    if (ov != nullptr && key != 0) {
        std::error_code ec(ret == FALSE ? cx::system_error()
                                        : std::error_code());
        overlapped_ex *pov = static_cast<overlapped_ex*>(ov);
        descriptor_type descriptor(pov->descriptor);
        pov->unhold();
        switch (pov->type) {
        case cx::io::pollin:
        case cx::io::pollout:
            return handle_event(descriptor, pov->type, ec, bytes_transferred);
        case cx::io::pollop: {
			overlapped_op* pov_op = static_cast<overlapped_op*>(pov);
            return handle_event(descriptor, pov_op->operation, ec,
                                bytes_transferred);
        }
...
int completion_port::handle_event(const descriptor_type &descriptor,
                                  operation_type *op, const std::error_code &ec,
                                  DWORD byte_transferred) {
    if (op) {
        op->set(ec, byte_transferred);
        if (op->complete(descriptor)) {
...
cx::slist<completion_port::operation_type>
completion_port::drain_ops(const completion_port::descriptor_type &descriptor,
                           const std::error_code &ec) {
