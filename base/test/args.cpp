...
struct option{
    ...
    option( Ts&& ... ts ){
        //(void)cx::basic_arg_expend{0, (set(ts), 0)...};
		CX_EXPAND_ARGS(set, ts);
    }

    void set( const keywords::name& arg ){
        this->name = arg.value;
    }

    void set( const keywords::age& arg){
        this->age = arg.value;
    }
	void set(const keywords::name1& arg) {
		this->name1 = arg.value;
	}
	std::string name;
	std::string name1;
	int age;
	/*
    void set( const keywords::same_type_0_t& arg){
        this->age = arg.value;
    }
     void set( const keywords::same_type_1_t& arg){
        this->age = arg.value;
    }
...
TEST(args,same_type){
    option opt(
        keywords::name("name") , 
        keywords::age(1) ,
        keywords::same_type_0_t(0) , 
        keywords::same_type_1_t(1));
