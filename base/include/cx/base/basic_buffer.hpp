 ...
 */
template <typename T, typename AllocatorType, typename IntType> class block {
public:
    using value_type = T;
    using int_type = IntType;

    block(const int_type size) : _ref_count(1), _size(size) {}
...
public:
    ...
    basic_buffer(void) noexcept : _block(nullptr), _rd_pos(0), _wr_pos(0) {}

    basic_buffer(const int_type sz) : basic_buffer() {
        _block = reinterpret_cast<block_type *>(AllocatorType::alloc(
            sizeof(block_type) + (sizeof(value_type) * sz)));
    ...
    value_type *rdptr(void) const {
        return _block ? _block->base() + _rd_pos : nullptr;
    }

    int_type rdptr(int_type n) {
        int_type offset =
            (n >= 0 ? std::min(n, rdsize()) : std::max(n, _rd_pos * -1));
        _rd_pos += offset;
        return offset;
    }
    ...
    value_type *wrptr(void) {
        return _block ? _block->base() + _wr_pos : nullptr;
    }

    int_type wrptr(int_type n) {
        int_type offset =
            (n >= 0 ? std::min(n, wrsize()) : std::max(n, rdsize() * -1));
        _wr_pos += offset;
        return offset;
    }
    ...
    void reserve(const int_type sz) {
        ...
        if ((size() - rdsize() >= sz) && _block && _block->use_count() == 1) {
    ...
    void commit(const int_type sz) { wrptr(sz); }
...
basic_buffer<T, AllocatorType, IntType>
deepcopy(basic_buffer<T, AllocatorType, IntType> &rhs) {
...
template <typename T, typename AllocatorType, typename IntType>
basic_buffer<T, AllocatorType, IntType> &
operator<<(basic_buffer<T, AllocatorType, IntType> &buf, const char *msg) {
